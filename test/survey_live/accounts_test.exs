defmodule SurveyLive.AccountsTest do
  use SurveyLive.DataCase, async: true

  import SurveyLive.AccountsFixtures
  import SurveyLive.UsersFixtures

  alias SurveyLive.Accounts
  alias SurveyLive.Accounts.Account
  alias SurveyLive.Accounts.Membership
  alias SurveyLive.Accounts.Invitation
  ...
  describe "accounts" do
    ...
    test "can_be_accessed_by_a_user?/2 returns true if its a users personal account" do
      %{accounts: [account]} = user = SurveyLive.Users.with_memberships(user_fixture())
